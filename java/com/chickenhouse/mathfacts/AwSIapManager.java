...
import com.chickenhouse.mathfacts.PurchaseDataSource.PurchaseStatus;
...
public class AwSIapManager {
    ...
    private static final String TAG = "AwSIAPManager";

    final private Context context;
    final private InAppPurchaseActivity mainActivity;
    private UserIapData userIapData;
    final private PurchaseDataSource dataSource;

    public AwSIapManager(final InAppPurchaseActivity mainActivity) {
    ...
    public void setAmazonUserId(final String newAmazonUserId, final String newAmazonMarketplace) {
        ...
        if (newAmazonUserId == null) {
            ...
            if (userIapData != null) {
                userIapData = null;
                mainActivity.updateEmailsInView(0);
            }
            ...
            mainActivity.updateEmailsInView(userIapData.getRemainingEmails());
        }
    }
    ...
    public void enablePurchaseForSkus(final Map<String, Product> productData) {
        if (productData.containsKey(MySku.ADDEMAILS.toString())) {
            mainActivity.enableBuyButton();
        }
    }
    ...
    public void disablePurchaseForSkus(final Set<String> unavailableSkus) {
        if (unavailableSkus.contains(MySku.ADDEMAILS.toString())) {
            mainActivity.disableBuyButton();
        }
    }
     ...
     * @param userData
     */
    public void handleConsumablePurchase(final Receipt receipt, final UserData userData, summaryDBHelper sumDB, SKUHelper skuDB) {
        try {
            if (receipt.isCanceled()) {
                revokeConsumablePurchase(receipt, userData, sumDB);
            } else {
                // We strongly recommend that you verify the receipt server-side
                if (!verifyReceiptFromYourService(receipt.getReceiptId(), userData, skuDB)) {
                    ...
                    System.out.println("Throws Can't be verified!!");
                    return;
                }else{
                    System.out.println("Purchase can be verified!");
                }
                if (receiptAlreadyFulfilled(receipt.getReceiptId(), userData)) {
                    ...
                    System.out.println("Already Fulfilled Notifying Amazon again.");
                    PurchasingService.notifyFulfillment(receipt.getReceiptId(), FulfillmentResult.FULFILLED);
                    return;
                }else{
                    System.out.println("This purchase has not been fulfilled!!!!!!");
                }


                System.out.println("Granting Consumable Purchase!");
                grantConsumablePurchase(receipt, userData, sumDB);
            }
            return;
     ...
     * @param userData
     */
    public void handleReceipt(final Receipt receipt, final UserData userData, summaryDBHelper sumDB, SKUHelper skuDB, final String adminEmail) {
        switch (receipt.getProductType()) {
        case CONSUMABLE:
            // try to do your application logic to fulfill the customer purchase
            try{
                createPurchase(receipt.getReceiptId(), userData.getUserId(), adminEmail);
                System.out.println("Should have Created A PURCHASE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            }catch(Exception e){
                System.out.println("Something went wrong with creating a purchase");

            }



            handleConsumablePurchase(receipt, userData, sumDB, skuDB);
            break;
        case ENTITLED:
    ...
    public boolean consumeEmailPurchase(summaryDBHelper sumDB) {
        if (userIapData == null) {
            mainActivity.showMessage("user not logged in to amazon marketplace!");
        }
        if (userIapData.getRemainingEmails() <= 0) {
            mainActivity.showMessage("You don't have emails remaining, buy more to help your students develop!");
            return false;
        }
        ...
        userIapData.setRemainingEmails(sumDB.getNumOfEmailsRemain());
        saveUserIapData();
        return true;
    }
    ...
    */
    ...
    /**
     * Update the available email numbers
     */
    public void unlockEmails(summaryDBHelper sumDB) {


        mainActivity.updateEmailsInView(sumDB.getNumOfEmailsRemain());
     ...
     * @param userData
     */
    private void revokeConsumablePurchase(final Receipt receipt, final UserData userData, summaryDBHelper sumDB) {
        int initialNumOfEmails = 0;
        int addtnlEmails = 5000;
        try {
            try {
                initialNumOfEmails = sumDB.getNumOfEmailsRemain();
            }catch (Exception e){
                Log.println(1,"Summary DB","Failed to determine initial number of emails in DB");
            }

            // Used max function because the end user may not have as many emails as I need to revoke
            int finalNumOfEmails = max(initialNumOfEmails - addtnlEmails,0);

            sumDB.insertSummary("EmailAllotment", "none", -1, -1, -1, -1, -1, finalNumOfEmails , "999");
            sumDB.close();
        }catch (Exception e){
            Log.println(1,"Summary DB","Failed insertSummary Statement. Attempting to inlcude additional " + addtnlEmails + " emails.");
        }
    ...
    private UserIapData reloadUserData(final String amazonUserId, final String amazonMarketplace) {
        ...
        userIapData.setRemainingEmails(orangesSharedPreference.getInt(REMAINING, 0));
        return userIapData;
    ...
    private void saveUserIapData() {
        ...
        try {
            final SharedPreferences orangesSharedPreference = context.getSharedPreferences("ADDEMAILS_" + userIapData.getAmazonUserId(),
                                                                                           Context.MODE_PRIVATE);
            final Editor editor = orangesSharedPreference.edit();
            editor.putInt(REMAINING, userIapData.getRemainingEmails());
            editor.commit();
        } catch (final Throwable e) {
     ...
     * @param userData
     */
    private void grantConsumablePurchase(final Receipt receipt, final UserData userData, summaryDBHelper sumDB) {
        try {
            // following sample code is a simple implementation, please
            // implement your own granting logic thread-safe, transactional and
            // robust
            ...
            if (updatePurchaseStatus(receipt.getReceiptId(), PurchaseStatus.PAID, PurchaseStatus.FULFILLED)) {
                // Update number of Emails available
                System.out.println("updatedPurchaseStatus -> Complete... attempt to update number of emails.");
                ...
                try {
                    ...
                    sumDB.insertSummary("EmailAllotment", "none", -1, -1, -1, -1, -1, initialNumOfEmails + addtnlEmails, "999");
                    sumDB.close();
                }catch (Exception e){
                    Log.println(1,"Summary DB","Failed insertSummary Statement. Attempting to include additional " + addtnlEmails + " emails.");
                }
     ...
     * @return
     */
    private boolean verifyReceiptFromYourService(final String receiptId, final UserData userData, SKUHelper skuDB) {


        Boolean newReciept = skuDB.exists(receiptId);

        System.out.println("The Data Base has this receipt " + newReciept.toString());
    ...
    private boolean receiptAlreadyFulfilled(final String receiptId, final UserData userData) {
        ...
        System.out.println("receiptRecord is " + receiptRecord.toString() + "" +
                " receiptRecord.getStatus() = " + receiptRecord.getStatus().toString());
     ...
     */
    private void createPurchase(final String receiptId, final String userId, final String adminEmail) {

        dataSource.createPurchase(receiptId, userId, PurchaseStatus.PAID, adminEmail);
    }
